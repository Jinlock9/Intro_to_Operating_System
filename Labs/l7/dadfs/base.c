...
static void dadfs_sb_sync(struct super_block *vsb)
{
...
static struct dadfs_inode *dadfs_inode_search(struct super_block *sb,
...
static void dadfs_inode_add(struct super_block *vsb, struct dadfs_inode *inode)
{
	struct dadfs_super_block *sb = DADFS_SB(vsb);
	struct buffer_head *bh;
...
static int dadfs_sb_get_a_freeblock(struct super_block *vsb, uint64_t * out)
{
	struct dadfs_super_block *sb = DADFS_SB(vsb);
	int i;
	int ret = 0;
...
static struct dadfs_inode *dadfs_get_inode(struct super_block *sb,
					  uint64_t inode_no)
{
...
static ssize_t dadfs_read(struct file * filp, char __user * buf, size_t len,
		      loff_t * ppos)
{
...
static int dadfs_inode_save(struct super_block *sb, struct dadfs_inode *sfs_inode)
{
	struct dadfs_inode *inode_iterator;
	struct buffer_head *bh;
 ...
 * Simplest
 */
static void dadfs_destory_inode(struct inode *inode)
{
...
static int dadfs_fill_super(struct super_block *sb, void *data, int silent)
{
