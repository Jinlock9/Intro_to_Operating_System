...
QueryResult::Ptr AddQuery::execute(Table &table) {
  ...
  try {
    ...
    if (result.second) {
      ...
      // threadSize += (table.size() % threadNum != 0);
      ...
      for (unsigned int i = 0; i < threadNum - 1; i++) {
        // safe: all tasks will be completed before return
        // &task will not cause dangling reference
        futures.push_back(
